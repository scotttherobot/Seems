...
CREATE TABLE `enrollments` (
  ...
  `enroll_date` int(11) NOT NULL,
  `status` enum('ENROLLED','DROPPED') NOT NULL DEFAULT 'ENROLLED',
  `notes` text,
  PRIMARY KEY (`id`),
  UNIQUE KEY `studentid_2` (`studentid`,`sectionid`),
  KEY `sectionid` (`sectionid`),
  KEY `studentid` (`studentid`),
  CONSTRAINT `enrollment_studentid_fk` FOREIGN KEY (`studentid`) REFERENCES `students` (`userid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `enrollment_section_fk` FOREIGN KEY (`sectionid`) REFERENCES `sections` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `sections` (
  ...
  CONSTRAINT `sections_teacherid_fk` FOREIGN KEY (`teacherid`) REFERENCES `teachers` (`userid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2014-02-10 12:13:54
